...
def create_permission():
    for permission_name in dir(PermissionEnum):
        if permission_name.startswith("_"):
            continue
...
def create_role():
    #稽查
    inspector = RoleModel(name = "稽查",desc = "负责审核帖子和评论否合法合规")
    ...
    operator = RoleModel(name = "运营" ,desc = "负责网站的正常运营")
    ...
    administrator = RoleModel(name="管理员",desc = "负责网站的整个运营")
...
def create_test_user():
    ...
    zhangsan = UserModel(username="张三",email="zhangsan@zlkt.ne", password="111111",is_staff=True,role=admin_role)
...
def create_admin(username,email,password):
    admin_role = RoleModel.query.filter_by(name = "管理员")
